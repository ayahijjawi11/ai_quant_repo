...
def enforce_fair_partial_policy(df: pd.DataFrame, supply_mw: float) -> pd.DataFrame:
    ...
    out = out.sort_values(by=["priority_level", "base_score"], ascending=[False, False]).copy()
...
def run_one_hour(hour: int = 12, supply_ratio: float = 0.35, n_sample: int = 12, seed: int = 42):
    ...
    # ---------- Quantum-inspired selection (0/1) ----------
    out_q_sel, best_score = solve_quantum_inspired(
        df_hour, supply_mw,
        enforce_fairness=True,      # fairness_ok (critical 
    ...
    #  selected (decision_x desc) priority score
    ...
    print(f"Hour={hour} | Supply limit={supply_mw:.2f} MW | Used={q_used:.2f} MW\n")

    cols = ["region", "zone", "facility_type", "predicted_demand_mw", "priority_level",
            "outage_risk", "allocation_level", "allocated_mw", "unmet_mw", "score"]

    # لازم نعرض جدول quantum مرتب priority ثم score مثل الصورة
    out_show = out_q.sort_values(by=["priority_level", "score"], ascending=[False, False]).copy()
    print(out_show[cols])
